...
import { useGetJobsQuery, useCreateJobMutation, useUpdateJobMutation, Job } from "@/store/services/recruitmentApi";
import { SVGLoaderFetch, NoRecordFound } from "@/components/ui/options";
import { RecruitmentJobsSkeleton } from "@/components/ui/skeleton/recruitment-skeletons";
...
export default function RecruitmentJobsPage() {
 ...
 const { data, isLoading, error } = useGetJobsQuery({ search });
 const [createJob, { error: createError }] = useCreateJobMutation();
 const [updateJob, { error: updateError }] = useUpdateJobMutation();

 const [isCreateDrawerOpen, setIsCreateDrawerOpen] = React.useState(false);
 const [isSuccessModalOpen, setIsSuccessModalOpen] = React.useState(false);
 const router = useRouter();

 useApiError(!!error, error, "Failed to load jobs");
 useApiError(!!createError, createError, "Failed to create job");
 useApiError(!!updateError, updateError, "Failed to update job");
 ...
 const handleAddJob = async (newJob: any) => {
  try {
   await createJob({
    ...
    quantity: parseInt(newJob.quantity),
    closingDate: newJob.closingDate,
    description: newJob.description,
    invitedMembers: newJob.invitedMembers,
    workflowStages: newJob.workflowStages.map((s: any) => ({ name: s.name, isLocked: s.isLocked }))
   }).unwrap();
 ...
 return (
  <div className="flex flex-col gap-8 p-2 md:p-8 min-h-full">
   ...
   <div className="flex flex-col gap-4">
    {isLoading ? (
     <RecruitmentJobsSkeleton />
    ) : jobs.length === 0 ? (
     <div className="flex justify-center py-10 bg-white dark:bg-gray-950 border border-gray-300 dark:border-gray-800 rounded-3xl">
      <NoRecordFound colSpan={1} text="No jobs found." />
     </div>
    ) : (
     jobs.map((job) => {
       ...
       >
        <div className="flex items-start justify-between gap-6">
         ...
         <div className="flex flex-col gap-3 flex-1">
          ...
          {/* Department · Office */}
          <p className="text-xs font-semibold text-gray-400 dark:text-gray-505">
           {job.department}
